...
contract MoodNFTTest is Test {
    ...
    function testViewTokenURI() public {
        ...
        // Mint an NFT to the test contract's address
        moodNft.mintNft("");

        // The first token ID should be 0
        uint256 tokenId = 0;

        // Get the token URI for the minted token
        string memory uri = moodNft.tokenURI(tokenId);

        // Assert that the token URI is not empty (it should contain the base64-encoded JSON)
